...
# 数据集
enc_inputs = torch.load('./dataset/enc_input.pkl')
dec_inputs = torch.load('./dataset/dec_input.pkl')
dec_outputs = torch.load('./dataset/dec_output.pkl')

enc_inputs = enc_inputs[:100]
dec_inputs = dec_inputs[:100]
dec_outputs = dec_outputs[:100]
...
src_len = 60  # enc_input max sequence length
tgt_len = 81  # dec_input(=dec_output) max sequence length
...
print(enc_inputs[0], '->', ''.join([idx2word[n.item()] for n in predict.squeeze()]))


text = 'PARIS is a city'
enc_inputs = [src_vocab[t] for t in text.split(' ')]
enc_inputs = torch.from_numpy(np.array(enc_inputs)).unsqueeze(0).type(torch.LongTensor)
greedy_dec_input = greedy_decoder(model, enc_inputs[0].view(1, -1).cuda(), start_symbol=tgt_vocab["<S>"])
predict, _, _, _ = model(enc_inputs[0].view(1, -1).cuda(), greedy_dec_input)
predict = predict.data.max(1, keepdim=True)[1]
print(enc_inputs[0], '->', ''.join([idx2word[n.item()] for n in predict.squeeze()]))
...
import numpy as np
with torch.no_grad():
    greedy_dec_input = greedy_dec_input.cpu()
greedy_dec_input = np.array(greedy_dec_input)
ans = [idx2word[i] for i in greedy_dec_input[0]]
''.join(ans)