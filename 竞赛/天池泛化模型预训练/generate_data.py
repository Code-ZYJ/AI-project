...
def split_dataset(dev_data_cnt=5000):
    for e in ['TNEWS', 'OCNLI', 'OCEMOTION']:
        cnt = 0
        with open('./tianchi_datasets/' + e + '/total.csv','rb') as f:
            with open('./tianchi_datasets/' + e + '/train.csv', 'wb') as f_train:
                with open('./tianchi_datasets/' + e + '/dev.csv', 'wb') as f_dev:
                    for line in f:
                        cnt += 1
                        if cnt <= dev_data_cnt:
                            f_dev.write(line)
...
def generate_data():
    ...
    for e in ['TNEWS', 'OCNLI', 'OCEMOTION']:
        ...
        with open('./tianchi_datasets/' + e + '/total.csv','rb') as f:
            for line in f:
                label = line.decode().strip().split('\t')[-1]
    ...
    for k, v in label_set.items():
        print(k, v)
    with open('./tianchi_datasets/label.json', 'wb') as fw:
        fw.write(json.dumps(label_set).encode())
    ...
    for k, v in label_weight_set.items():
        print(k, v)
    with open('./tianchi_datasets/label_weights.json', 'wb') as fw:
        fw.write(json.dumps(label_weight_set).encode())
    
    for e in ['TNEWS', 'OCNLI', 'OCEMOTION']:
        for name in ['dev', 'train']:
            with open('./tianchi_datasets/' + e + '/' + name + '.csv','rb') as fr:
                with open('./tianchi_datasets/' + e + '/' + name + '.json', 'wb') as fw:
                    json_dict = dict()
                    for line in fr:
                        tmp_list = line.decode().strip().split('\t')
                    ...
                    fw.write(json.dumps(json_dict).encode())
