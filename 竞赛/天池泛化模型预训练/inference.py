...
def test_csv_to_json():
    for e in ['TNEWS', 'OCNLI', 'OCEMOTION']:
        with open('./tianchi_datasets/' + e + '/test.csv','r',encoding='utf-8') as fr:
...
def inference_warpper(tokenizer_model):
    ocnli_test = dict()
    with open('./tianchi_datasets/OCNLI/test.json','r',encoding='utf-8') as f:
        for line in f:
            ocnli_test = json.loads(line)
            break
        
    ocemotion_test = dict()
    with open('./tianchi_datasets/OCEMOTION/test.json','r',encoding='utf-8') as f:
        for line in f:
            ocemotion_test = json.loads(line)
            break
        
    tnews_test = dict()
    with open('./tianchi_datasets/TNEWS/test.json','r',encoding='utf-8') as f:
        for line in f:
            tnews_test = json.loads(line)
            break
        
    label_dict = dict()
    with open('./tianchi_datasets/label.json','r',encoding='utf-8') as f:
        for line in f:
    ...
    inference('./submission/ocnli_predict.json', ocnli_test, model, tokenizer, label_dict['OCNLI'], 'ocnli', 'cuda:0', 64, True)
    inference('./submission/ocemotion_predict.json', ocemotion_test, model, tokenizer, label_dict['OCEMOTION'], 'ocemotion', 'cuda:0', 64, True)
    inference('./submission/tnews_predict.json', tnews_test, model, tokenizer, label_dict['TNEWS'], 'tnews', 'cuda:0', 64, True)
